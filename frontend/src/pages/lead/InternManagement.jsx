...
const InternManagement = () => {
    ...
    const handleApprove = async (id) => {
        try {
            ...
            setPendingInterns(prev => prev.filter(u => u.id !== id));
        ...
        <DashboardLayout role="Lead">
            <div style={{ marginBottom: '2rem' }}>
                ...
                {pendingInterns.length > 0 && (
                    <div className="mb-6">
                        <h2 className="text-lg font-bold mb-4">Pending Approvals ({pendingInterns.length})</h2>
                        <div className="grid grid-responsive gap-4">
                            {pendingInterns.map(user => (
                                <Card key={user.id} className="p-4 border-l-4 border-warning" style={{ borderLeft: '4px solid var(--warning)' }}>
                                    <div className="flex justify-between items-start">
                                        <div>
                                            <h3 className="font-bold">{user.name}</h3>
                                            <p className="text-muted text-sm">{user.email}</p>
                                            <div className="mt-2 text-sm">
                                                <span className="badge badge-secondary mr-2">{user.role?.name || 'Intern'}</span>
                                                {user.technology && <span className="badge badge-primary">{user.technology.name}</span>}
                                            </div>
                                            <div className="mt-1 text-xs text-muted">Registered: {new Date(user.created_at).toLocaleDateString()}</div>
                                        </div>
